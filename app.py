...
def compute_kpis(region, people, organisations, events, payments, grants):
    ...
    volunteers = [p for p in region_people if 'Volunteer' in (p.get('type') or [])]
    steering_group_proxy = len(volunteers)
    ...
    for org in region_orgs:
        ...
        if org_type in ["University", "Trust", "Political/Parliamentary", "Media", "NHS/Social Prescriber"]:
    ...
    for g in grants:
        ...
        if org_link and org_link.get('id') in org_id_to_region:
            region_grants.append(g)
        elif region == "Global":
            region_grants.append(g)
            
    bids_submitted = sum(1 for g in region_grants if g.get('stage') == 'Submitted')
    funds_raised_grants = sum(float(g.get('amount') or 0) for g in region_grants if g.get('stage') == 'Won')
    ...
    walks_delivered = len([e for e in region_events if str(e.get('type')).lower() in ['walk', 'retreat']])
    ...
    return {
        ...
        "delivery": {
            ...
            "participants": sum(int(e.get('number_of_attendees') or 0) for e in region_events),
        ...
        "income": {
            ...
            "corporate_partners": ldp_counts.get('Business - Other', 0),
            "in_kind_value": 0 
        },
        "comms": {
            "press_releases": 0,
