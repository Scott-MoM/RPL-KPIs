...
def get_db_connection():
    ...
    try:
        ...
        if "supabase" in st.secrets:
            ...
            key = st.secrets["supabase"].get("SUPABASE_ANON_KEY") or st.secrets["supabase"].get("key")
        
        if not url:
            url = st.secrets.get("SUPABASE_URL") or st.secrets.get("url")
        if not key:
            key = st.secrets.get("SUPABASE_ANON_KEY") or st.secrets.get("key")
...
def create_user(name, email, password, roles, region):
    ...
    if DB_TYPE == 'supabase':
        try:
            ...
            # We catch weak password errors so it doesn't crash the dashboard for the admin
